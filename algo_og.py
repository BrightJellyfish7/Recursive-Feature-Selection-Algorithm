...
class algo:
	def __init__(self, adata, group1, group2, field_name="sample", plots=False, print_to_console=False, max_iterations=100, additive=True, flip_rate_percentage=0.01, pval_cutoff=0.05, seed=42):
		self.adata = adata
		self.group1 = group1
		self.group2 = group2
	...
	def filter_groups(self):
		...
		n1, n2 = self.adata_group1.n_obs, self.adata_group2.n_obs
		if n1 == 0 or n2 == 0:
			available = self.adata.obs[self.field_name].unique().tolist()
			raise ValueError(
				f"group1='{self.group1}' has {n1} cells, group2='{self.group2}' has {n2} cells "
				f"in field '{self.field_name}'. Available values: {available}"
			)
	...
	def ensemble_learner_plots(self):
		...
		sns.scatterplot(x=self.adata.obs.log1p_total_counts.values.flatten(),
						...
						hue=self.adata.obs[self.field_name])
	...
	def scanpy_de_original_groups(self, n_top=1000, method="wilcoxon"):
		...
		# Filter by p-value cutoff then take top n
