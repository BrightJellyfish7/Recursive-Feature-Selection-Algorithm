...
>>>>>>> 708fbe9 (GMM stabilization stopping point, correlated gene panel plot added, output of Algo genes and DE genes for testing on Enrichr)
...
class algo:
<<<<<<< HEAD
    def __init__(self, adata, group1, group2, field_name="sample", plots=False, max_iterations=100, additive=False):
        self.adata = adata
        self.group1 = group1
        self.group2 = group2
    ...
    def ensemble_learner(self):
        ...
        self.genes.append(self.df["genes"][0])  # append the top gene to self.genes
    ...
    def ensemble_recursion(self, iteration=1, additive=False):
        ...
        auc = 101
        moving_average = 200
        while (iteration <= self.max_iterations):# and (auc<moving_average): # mean change/moving average/always less than 100
            print(f"Iteration {iteration}")
            print(iteration, self.max_iterations, auc, moving_average)

            # Step 1: cluster + store labels in self.adata.obs
            gene = self.genes[-1]

            self.cluster_infectivity_gmm()
            gmm_key = f"gmm_{gene}"

            # Step 2: RF to predict cluster labels
            X = self.adata.X
            y = self.adata.obs[gmm_key].values  # new labels for the next training iteration from the gmm
            ...
            if iteration<=5:
                ...
                moving_average = sum(self.auc_scores[-5:])/5
                print(moving_average)
    ...
    def scanpy_de_original_groups(self, n_top=100, method="wilcoxon"):
        """
...
<<<<<<< HEAD
adata_alz = sc.read_h5ad("alz_data_reduced.h5ad")

adata_alz.shape

# run the recursive additive differential expression algorithm
...
model.auc_scores
model.gene_expression_log

model.rows

# check intersection between algorithm genes and regular differnential expression genes
hits, hits_df = model.intersect_genes_and_de()
hits
hits_df

# and (auc<moving_average)
